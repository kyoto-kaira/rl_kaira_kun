...
public class CrawlerAgent : Agent
{
    ...
    [Tooltip("Scene target Transform. If left null, the agent will not auto-find a target.")]
    public Transform Target; // Target instance created elsewhere (e.g., Camera's SpawnOnClicks)

    private Transform m_Target; // Cached reference
    ...
    public override void Initialize()
    {
        ...
        // Cache target created by external spawner (no tag auto-find)
        m_Target = Target;
    ...
    public override void OnEpisodeBegin()
    {
        ...
        body.rotation = Quaternion.Euler(8, 130, 3);
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // When no target, hold position by making the velocity goal zero.
        var desiredSpeed = (m_Target == null) ? 0f : TargetWalkingSpeed;
        // velocity we want to match
        var velGoal = cubeForward * desiredSpeed;
        //ragdoll's avg vel
    ...
    void FixedUpdate()
    {
        ...
        // a. Match target speed (or zero if no target to hold position)
        // This reward will approach 1 if it matches perfectly and approach zero as it deviates
        var desiredSpeed = (m_Target == null) ? 0f : TargetWalkingSpeed;
        var matchSpeedReward = GetMatchingVelocityReward(cubeForward * desiredSpeed, GetAvgVelocity());
    ...
    /// <summary>
    /// External spawner (e.g., Camera's SpawnOnClicks) can register the created target here.
    /// </summary>
    ...
    {
        // Refresh target if not yet assigned
        if (m_Target == null)
        {
            // If user assigned via inspector later
            if (Target != null) m_Target = Target;
        }

        // Fall back to body if we still don't have a target yet
        var targetForOrientation = m_Target != null ? m_Target : body;

        m_OrientationCube.UpdateOrientation(body, targetForOrientation);
